...
from Occ.utils.data_utils import normalize_pose
from Occ.utils.pose_utils import gen_clip_seg_data_np, get_ab_labels
...
class PoseSegDataset(Dataset):
    ...
    def __init__(self, path_to_json_dir, path_to_vid_dir=None, normalize_pose_segs=True, return_indices=False,
                 return_metadata=False, debug=False, return_global=True, evaluate=False, abnormal_train_path=None,pseudo_weight=None,mode='unsupervised',
                 **dataset_args):
        super().__init__()
        ...
        self.global_data, self.segs_score_np,self.segs_weight_np = \
            gen_dataset(path_to_json_dir, num_clips=num_clips, ret_keys=True,
                        ret_global_data=return_global,pseudo_weight=pseudo_weight,mode=mode, **dataset_args)
        self.segs_meta = np.array(self.segs_meta)

        # can put uncertainty to this labels
    ...
    def __getitem__(self, index):
        ...
        ret_arr += [self.segs_weight_np[sample_index]]
        return ret_arr
    ...
    def update_weights(self,weights):
        ...
        for i in range(len(self.metadata)):
            ...
            clip_id = item[1]
            start_key = item[3]
            if self.args['dataset'] == 'ShanghaiTech':
                clip_length = item[4]
                if scene_id < 10:
                    scene = '0' + str(scene_id)
                else:
                    scene = str(scene_id)
                if clip_id >= 10 and clip_id<100:
                    clip = '00'+str(clip_id) if clip_length == 4 else '0'+str(clip_id)
                elif clip_id<10:
                    clip = '00'+str(clip_id) if clip_length == 3 else '000'+str(clip_id)
                elif clip_id>=100:
                    clip = '0'+str(clip_id) if clip_length == 4 else '00'+str(clip_id)
            elif self.args['dataset'] == 'UBnormal':
                scene = str(scene_id)
                clip = clip_id
                start_key = int(start_key)
            video_name = scene+'_'+clip
            pseudo_weights.append(weights[video_name][start_key:start_key+self.seg_len])
        pseudo_weights = np.stack(pseudo_weights,axis=0)
        self.segs_weight_np = pseudo_weights
        print("pseudo_weights updated!")

    # def update_labels(self):
    #     ab_ratio = 0.05
    #     frame_num = self.segs_weight_np.shape[0]
    #     a_num = math.ceil(ab_ratio * frame_num)
    #     segs_weight_np = np.amin(self.segs_weight_np,axis=-1)
    #     ab_idx = np.argpartition(-segs_weight_np,a_num)[:a_num]
    #     label = np.ones(self.segs_data_np.shape[0])
    #     label[ab_idx] = -1
    #     self.labels = label
...
def get_Occ_dataset(trans_list, only_test=False,pseudo_weight=None):
    parser = init_parser()
    ...
    dataset_args = {'headless': args.headless, 'scale': args.norm_scale, 'scale_proportional': args.prop_norm_scale,
                    ...
                    'train_seg_conf_th': args.train_seg_conf_th, 'specific_clip': args.specific_clip}
    dataset = dict()
    splits = ['train','train_test', 'test'] if not only_test else ['test']
    for split in splits:
        # when test,we don't transform
        ...
        if split == 'train':
            dataset[split] = PoseSegDataset(args.pose_path[split], path_to_vid_dir=args.vid_path[split],
                                    normalize_pose_segs=normalize_pose_segs,
                                    evaluate=evaluate,
                                    abnormal_train_path=abnormal_train_path,pseudo_weight=pseudo_weight,mode=args.mode,
                                    **dataset_args)
        else:
            dataset[split] = PoseSegDataset(args.pose_path[split], path_to_vid_dir=args.vid_path[split],
                                    normalize_pose_segs=normalize_pose_segs,
...
def gen_dataset(person_json_root, num_clips=None, kp18_format=True, ret_keys=False, ret_global_data=True,pseudo_weight=None,mode='unspervised',
                **dataset_args):
    segs_data_np = []
    segs_score_np = []
    segs_meta = []
    ...
    for person_dict_fn in tqdm(json_list):
        ...
        clip_segs_data_np, clip_segs_meta, clip_keys, single_pos_np, _, score_segs_data_np,weight_segs_data_np = gen_clip_seg_data_np(
        ...
        _, _, _, global_data_np, global_data, _, _ = gen_clip_seg_data_np(clip_dict, start_ofst, 1, 1, scene_id=scene_id,
                                                                       clip_id=clip_id,
                                                                       ret_keys=ret_keys,
        ...
        segs_weight_np.append(weight_segs_data_np)
    ...
    segs_weight_np = np.concatenate(segs_weight_np, axis=0)
    ...
    if ret_global_data:
        if ret_keys:
            return segs_data_np, segs_meta, person_keys, global_data_np, global_data, segs_score_np,segs_weight_np
        else:
            return segs_data_np, segs_meta, global_data_np, global_data, segs_score_np,segs_weight_np
    if ret_keys:
        return segs_data_np, segs_meta, person_keys, segs_score_np,segs_weight_np
    else:
        return segs_data_np, segs_meta, segs_score_np,segs_weight_np

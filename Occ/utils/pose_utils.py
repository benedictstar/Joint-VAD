...
def gen_clip_seg_data_np(clip_dict, start_ofst=0, seg_stride=4, seg_len=12, scene_id='', clip_id='', ret_keys=False,
                         global_pose_data=[],pseudo_weight=None,mode='unsupervised', dataset="ShanghaiTech"):
    """
    ...
    weight_segs_data = []
    person_keys = {}
    # add weight from other model to train
    for idx in sorted(clip_dict.keys(), key=lambda x: int(x)):
        ...
        if pseudo_weight is not None:
            # modify 
            single_weight = pseudo_weight[scene_id+'_'+clip_id]

        person_keys[key] = sing_pose_keys
        curr_pose_segs_np, curr_pose_segs_meta, curr_pose_score_np,curr_pose_weight_np = split_pose_to_segments(sing_pose_np,
                                                                                            sing_pose_meta,
                                                                                            sing_pose_keys,
        ...
        weight_segs_data.append(curr_pose_weight_np)
    ...
    if len(pose_segs_data) == 0:
        ...
        weight_segs_data_np = np.empty(0).reshape(0, seg_len)
    else:
        pose_segs_data_np = np.concatenate(pose_segs_data, axis=0)
        score_segs_data_np = np.concatenate(score_segs_data, axis=0)
        weight_segs_data_np = np.concatenate(weight_segs_data, axis=0)
    global_pose_data_np = np.concatenate(global_pose_data, axis=0)
    del pose_segs_data
    # del global_pose_data
    if ret_keys:
        return pose_segs_data_np, pose_segs_meta, person_keys, global_pose_data_np, global_pose_data, score_segs_data_np,weight_segs_data_np
    else:
        return pose_segs_data_np, pose_segs_meta, global_pose_data_np, global_pose_data, score_segs_data_np,weight_segs_data_np
...
def split_pose_to_segments(single_pose_np, single_pose_meta, single_pose_keys, start_ofst=0, seg_dist=6, seg_len=12,
                           scene_id='', clip_id='', single_score_np=None, single_weight=None,dataset="ShanghaiTech",mode='unsupervised'):
    ...
    pose_segs_meta = []
    # add weight
    pose_segs_weight = np.empty([0, seg_len])
    ...
    for seg_ind in range(num_segs):
        ...
        if is_seg_continuous(single_pose_keys_sorted, start_key, seg_len):
            ...
            if dataset == "UBnormal":
                ...
                pose_segs_meta.append([int(scene_id), int(clip_id), int(single_pose_meta[0]), int(start_key), int(len(clip_id))])
            if single_weight is not None:
                curr_weight = single_weight[start_key:start_key + seg_len].reshape(1, seg_len)
                pose_segs_weight= np.append(pose_segs_weight, curr_weight, axis=0)
            # when testing,we don't use weights
            # only change the weights for training set
            else:
                if mode == 'unsupervised':
                    # use beta distribution to initialize the weights as default
                    alpha = 5
                    beta = 1
                    samples = np.random.beta(alpha, beta, seg_len)
                    pose_segs_weight= np.append(pose_segs_weight, np.array(samples).reshape(1,seg_len), axis=0)
                    # all 1 to initialize
                    # pose_segs_weight= np.append(pose_segs_weight, np.ones((1,seg_len)), axis=0)
    assert len(pose_segs_np) == pose_segs_weight.shape[0]
    return pose_segs_np, pose_segs_meta, pose_score_np, pose_segs_weight

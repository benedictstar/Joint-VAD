...
class Model(nn.Module):
    def __init__(self, n_features, batch_size,extractor):
    ...
    def forward(self, inputs,largest=False):

        k_abn = self.k_abn
        k_nor = self.k_nor
        # c3d need to be adpated
        if self.extractor == 'c3d':
            inputs = inputs[:,:,:,::2]
        ...
        idx_bottom_abn = torch.topk(afea_magnitudes_drop, k_abn, dim=1,largest=False)[1]
        idx_bottom_abn_feat = idx_bottom_abn.unsqueeze(2).expand([-1, -1, abnormal_features.shape[2]])
        ...
        abn_bottom_feature = torch.zeros(0,device=inputs.device)
        for abnormal_feature in abnormal_features:
            feat_select_abn = torch.gather(abnormal_feature, 1, idx_abn_feat)   # top 3 features magnitude in abnormal bag
            feat_bottom_abn = torch.gather(abnormal_feature, 1, idx_bottom_abn_feat)
            total_select_abn_feature = torch.cat((total_select_abn_feature, feat_select_abn))
            abn_bottom_feature = torch.cat((abn_bottom_feature,feat_bottom_abn))
        ...
        return score_abnormal, score_normal, feat_select_abn, feat_select_normal, abn_bottom_feature, feat_select_abn, scores, feat_select_abn, feat_select_abn, feat_magnitudes